...
void cControlGameEngine::UpdateAllAnimations()
{
    if (mAnimationManager != NULL)
        this->mAnimationManager->UpdateAnimations();
}

void cControlGameEngine::PlayAnimation(std::string modelName, std::string animationName, float frameRateIncrement)
{
    ...
    {
        sModelDrawInfo* animModel = g_pFindModelInfoByFriendlyName(modelName);

        if(!this->mAnimationManager->AddAnimationCommand(animMesh, animModel, animationName, frameRateIncrement))
